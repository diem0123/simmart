...
<meta property="og:url" itemprop="url" content="http://cungnhaulen.top/san-pham/{{$row->slug}}" />
...
<div class="row" style="margin-top: 10px;">
	...
	<div class="col-md-2 hidden-xs"><span style="float:left;margin-top:7px;"><input name="so10" id="10so" type="checkbox" value="10">Sim 10 số</span></div>
</div>
	...
	$(document).ready(function () {
		$(".sapxep").change(function() {
            ...
            $.ajax({
            	type: 'GET',
            	url: "{{ route('proajax')}}",
            	data: {id: id,cat:cat,price:price},
...
<script language="javascript">
	document.getElementById('10so').onclick = function(e){
		...
		$.ajax({
				type: 'GET',
				url: "{{ route('ajaxchecksp')}}",
				data: {id: id,cat:cat,loai:loai,price:price},
				success: function(data) {
					$(".danhsach").html(data);
            }
        });
...
<script>
	$(document).ready(function () {
		$("#price").change(function() {
            ...
            $.ajax({
            	type: 'GET',
            	url: "{{ route('proajaxprice')}}",
            	data: {id: id,cat:cat,loai:loai},
            	success: function(data) {
            		$(".danhsach").html(data);
                    //alert(data);
                }

@extends('masteradmin')
@section('title', 'Cập nhật số sim')
@section('noidung')
<div class="content-wrapper">
	<section class="content-header">
    <h1>
      Cập nhật sim {{$row->number}}
    </h1>
    </section>
					...
					<div class="row">
						<div class="col-md-4">
							<div class="form-group">
							    <label>Số sim</label>
							    <input value="{{old('number',$row->number)}}" type="text" class="form-control" name="number">
						...
						<div class="col-md-4">
							<div class="form-group">
								...
								<select name="catid" class="form-control">
									<option></option>
									@foreach($cat as $cate)
									<option value="{{$cate->id}}" <?php if(($cate->id)==($row->catid)){echo "selected";} ?>>{{$cate->name}}</option>
									@endforeach
								</select>
							</div>
							<div class="form-group">
								...
								<select name="nhamang" class="form-control">
									<option></option>
									@foreach($nm as $nms)
									<option value="{{$nms->id}}" <?php if(($nms->id)==($row->nhamang)){echo "selected";} ?>>{{$nms->name}}</option>
									@endforeach
							...
							<div class="form-group">
								...
								<select name="styleid" class="form-control">
									...
									<option value="{{$sty->id}}" <?php if(($sty->id)==($row->styleid)){echo "selected";} ?>>{{$sty->name}}</option>
									@endforeach
